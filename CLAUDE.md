...
`wrangler.jsonc` ships `SHARED_SECRET` as the placeholder `"YOUR_SECRET_KEY_HERE"`. Never commit a real value there. Use `wrangler secret put SHARED_SECRET` for production, and `.dev.vars` (gitignored) for local dev. The API authenticates via `Authorization: Bearer <SHARED_SECRET>`. The Worker **fails closed**: while `SHARED_SECRET` is unset or still the placeholder, every POST returns `503` (see `isSecretConfigured` in `src/index.ts`), so local dev needs a real `.dev.vars` secret.
