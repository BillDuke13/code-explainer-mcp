/**
 * Code Explainer MCP — a Cloudflare Worker that explains source code.
 *
 * The Worker exposes a single HTTP endpoint through the default `fetch` handler:
 *   - POST JSON `{ "method": "explainCode", "params": [code, language] }` with the
 *     header `Authorization: Bearer <SHARED_SECRET>` returns `{ "result": "<markdown>" }`.
 *   - GET returns a static HTML info page.
 *
 * Analysis runs entirely inside the Worker using regex/pattern matching — there are no
 * LLM calls and no external runtime dependencies. All logic lives in this file; `explainCode`
 * and its helpers are module-private and are reached only through the `fetch` handler below.
 */

// Binding types for the Worker, mirrored in worker-configuration.d.ts (regenerate with `npm run cf-typegen`).
interface Env {
 ...
 */
function explainCode(code: string, programming_language: string): string {
	...
	// Assemble the Markdown report returned to the caller.
	return [
...
function generateArchitectureDiagram(code: string, programming_language: string): string {
	...
	let classRegex, functionRegex, importRegex;
	...
	while ((match = functionRegex.exec(code)) !== null) {
		...
		for (const cls of classes) {
			const classCode = extractBlock(code, cls.index, programming_language);
	...
	for (const func of functions) {
		const functionCode = extractBlock(code, func.index, programming_language);

		// Check if this function calls other functions
		for (const otherFunc of functions) {
			if (func.name !== otherFunc.name && functionCode.includes(`${otherFunc.name}(`)) {
				relationships.push({
					from: func.name,
					to: otherFunc.name,
					type: 'calls',
...
function extractCoreFunctionality(code: string, programming_language: string): string {
	...
	const patterns: Record<string, RegExp[]> = {
		network: [/fetch\s*\(/i, /http/i, /request/i, /api/i, /url/i, /endpoint/i],
		ui: [/render/i, /component/i, /view/i, /display/i, /ui/i, /interface/i, /dom/i],
		dataProcessing: [/map\s*\(/i, /filter\s*\(/i, /reduce\s*\(/i, /transform/i, /convert/i, /parse/i],
		authentication: [/auth/i, /login/i, /password/i, /credential/i, /token/i, /permission/i],
		database: [/database/i, /db\./i, /query/i, /sql/i, /mongo/i, /store/i, /save/i, /repository/i],
		testing: [/test/i, /assert/i, /expect/i, /mock/i, /spec/i],
		algorithm: [/algorithm/i, /sort/i, /search/i, /calculate/i, /compute/i],
		fileSystem: [/file/i, /read/i, /write/i, /path/i, /directory/i, /folder/i],
	};
	...
	switch (primaryPurpose) {
		...
		case 'network':
			...
			if (/api/i.test(code) || /endpoint/i.test(code)) {
...
function extractComponents(
	...
	// Class declarations share the same shape across the supported languages.
	const classRegex = /class\s+(\w+)/g;
	let classMatch;
	while ((classMatch = classRegex.exec(code)) !== null) {
		const className = classMatch[1];
		const classCode = extractBlock(code, classMatch.index, programming_language);
		const description = generateComponentDescription(classCode, 'class');
		mainClasses.push({ name: className, description });
	}

	// Select function/method patterns by language; the name is capture group 1 (or 2 for the generic fallback).
	let functionRegex: RegExp;
	let arrowFunctionRegex: RegExp | null = null;
	switch (programming_language.toLowerCase()) {
		case 'python':
			functionRegex = /def\s+(\w+)/g;
			break;
		...
		case 'ts':
			functionRegex = /function\s+(\w+)/g;
			arrowFunctionRegex = /const\s+(\w+)\s*=\s*(?:\([^)]*\)|[^=]*)\s*=>/g;
			break;
		default:
			functionRegex = /function\s+(\w+)|def\s+(\w+)/g;
			arrowFunctionRegex = /const\s+(\w+)\s*=\s*(?:\([^)]*\)|[^=]*)\s*=>/g;
	}
	...
	while ((functionMatch = functionRegex.exec(code)) !== null) {
		...
		const functionCode = extractBlock(code, functionMatch.index, programming_language);
		const description = generateComponentDescription(functionCode, 'function');
		mainFunctions.push({ name: functionName, description });
	}

	// Arrow functions only apply to JavaScript/TypeScript (and the generic fallback).
	if (arrowFunctionRegex) {
		let arrowMatch;
		while ((arrowMatch = arrowFunctionRegex.exec(code)) !== null) {
			const functionName = arrowMatch[1];
			const functionCode = extractBlock(code, arrowMatch.index, programming_language);
			const description = generateComponentDescription(functionCode, 'function');
			mainFunctions.push({ name: functionName, description });
		}
	}
...
/**
 * Extracts a code block starting from a given position. Brace-delimited languages use brace
 * matching; indentation-based languages (Python) use indentation to find the block extent.
 *
 ...
 */
function extractBlock(code: string, startIndex: number, programming_language: string): string {
	// Indentation-delimited languages have no braces; fall back to indentation analysis.
	if (programming_language.toLowerCase() === 'python') {
		return extractIndentedBlock(code, startIndex);
	}
...
/**
 * Extracts an indentation-delimited block (e.g. a Python `def`/`class` body) starting at the
 * header keyword. The block runs to the first later non-blank line indented no deeper than the
 * header line; blank lines stay part of the block.
 *
 * @param code {string} The full source code
 * @param startIndex {number} The index of the block's header keyword
 * @return {string} The extracted code block
 */
function extractIndentedBlock(code: string, startIndex: number): string {
	const lineStart = code.lastIndexOf('\n', startIndex) + 1;
	let headerIndent = 0;
	while (lineStart + headerIndent < code.length && (code[lineStart + headerIndent] === ' ' || code[lineStart + headerIndent] === '\t')) {
		headerIndent++;
	}

	const headerLineEnd = code.indexOf('\n', startIndex);
	if (headerLineEnd === -1) return code.substring(startIndex);
	...
	while (cursor < code.length) {
		...
		const line = code.substring(cursor, lineEnd);
	...
	return code.substring(startIndex, blockEnd);
}
	...
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		if (request.method === 'POST') {
			...
			try {
				...
				// Validate the method and parameter shape before dispatching.
				...
				const [code, language] = body.params;
				if (typeof code !== 'string' || typeof language !== 'string') {
					return new Response('Invalid method or parameters', { status: 400 });
				}
				if (code.length > MAX_CODE_LENGTH) {
					return new Response(`Code exceeds the maximum length of ${MAX_CODE_LENGTH} characters`, { status: 413 });
				}
		...
		// Serve the static info page for GET/HEAD; other methods are rejected.
